...
- (instancetype)initWithSession:(id<AlfrescoSession>)session
{
    ...
    // we can't access Data Dictionary on Cloud so just return nil
    if ([session isKindOfClass:[AlfrescoCloudSession class]])
    {
        return nil;
    }

    return (id)[[AlfrescoClientBasedConfigService alloc] initWithSession:session];
}

- (instancetype)initWithDictionary:(NSDictionary *)parameters
{
    return (id)[[AlfrescoClientBasedConfigService alloc] initWithDictionary:parameters];
}
